...
class Zend_Controller_Action_Helper_Contenedores extends Zend_Controller_Action_Helper_Abstract
{
    function printContenedores(){
        ...
        $rows = $contenedor->fetchAll($contenedor->select()
               ->from(array('c'=>'Contenedores'),array('c.CodContenedor','c.NombreContenedor','c.CodDestino','count(co.CodContenedor) as cantidad'))
        ...
        foreach ($rows as $contenedor) {
            ...
            echo "<li class='ui-state-default'><a class='peque_link' href='listar/details?CodContenedor=".$contenedor["CodContenedor"]."'>".$contenedor["NombreContenedor"]."<br><p class='peque'>".$centro[0]["NombreCentro"]."<br/> <span style='color:#11FF11'>".$contenedor["cantidad"]." Equipo(s)</span></p></a></li>";
        }
        echo "</ol>";
    }
    ...
    function printContenedorOrdenes($CodEnvio){
        ...
        $cont_envio .=  '<table style="font-size: 75%;" border="1"><tr width="7px"><th>Numero</th><th>Orden</th><th>Orden Externa</th><th>Marca</th><th>Modelo</th><th>Status</th><th>Observaciones</th><th>Reparacion</th><th>Serial Decimal</th><th>Serial Hexadecimal</th><th>Serial de Salida</th><th>Accesorio</th></tr>';
        $i=1;
        foreach ($Ordenes as $orden) {

            $cont_envio .=  '<tr><td>'.$i.'</td><td>'.$orden['CodOrden'].'</td><td>'.$orden['OrdenExterna'].'</td><td>'.$orden['NombreMarca'].'</td><td>'.$orden['NombreModelo'].'</td><td>'.$orden['NombreStatus'].'</td><td>Observacion</td><td>'.$orden['reparacion'].'</td><td>'.$orden['Serial1'].'</td><td>'.$orden['Serial2'].'</td>';
    ...
    function printNumeroModelos($CodEnvio){

        $db = Zend_Db_Table::getDefaultAdapter();  

        $stmt = $db->query("SELECT vw_Ordenes.NombreModelo, count(vw_Ordenes.NombreModelo) as cantidad
        FROM vw_Ordenes INNER JOIN
        Paquetes_Ordenes ON vw_Ordenes.CodOrden = Paquetes_Ordenes.CodOrden INNER JOIN
        StatusOS ON vw_Ordenes.CodStatus = StatusOS.CodStatus INNER JOIN
        vw_EnviosPaquetes ON Paquetes_Ordenes.CodPaquete = vw_EnviosPaquetes.CodPaquete
        WHERE (vw_EnviosPaquetes.CodEnvio =".$CodEnvio.") GROUP BY vw_Ordenes.NombreModelo"
        ...
        $cont_envio =  '<table style="font-size: 75%;" border="1"><tr width="7px"><th>Numero</th><th>NombreModelo</th><th>Cantidad</th></tr>';
        $i=1;
        foreach ($Ordenes as $orden) {

            $cont_envio .=  '<tr><td>'.$i.'</td><td>'.$orden['NombreModelo'].'</td><td>'.$orden['cantidad'].'</td></tr>';
            $i++;
        }
        $cont_envio .=  '</table>';
        return $cont_envio;
