...
class Contenedores_Model_Contenedor
{
	...
	public function CreatePaquete($CodContenedor,$NoGuia,$Observaciones,$Transporte,$usuario) {
		...
		try{
			...
			$stmt = $db->query("If Exists (Select CodPaquete from Paquetes Where CodCentro=160 And CodCentroDestino=".$rows["CodDestino"]." And Tipo=1 And Status=1)
								begin
									...
									Values (160, ".$rows["CodDestino"].", 1, '$usuario', GetDate(), 1, 0)
								");
	        ...
	        foreach ($ordenes as $orden) {
				$stmt3 = $db->query("exec sp_InsertPaqueteOrden ".$CodPaquete.",".$orden['CodOrden'].",'$usuario'");
	        }
	        //creamos el envio
	        $sql="declare @p1 int;EXECUTE sp_InsertEnvio @p1 output,160,".$rows["CodDestino"].",'$usuario' ";
			$stmt4 = $db->query($sql);
			...
			$stmt7 = $db->query("exec sp_UpdateEnvio ".$CodEnvio.",".$Transporte.",".$NoGuia.",".$Observaciones.",'$usuario'");
